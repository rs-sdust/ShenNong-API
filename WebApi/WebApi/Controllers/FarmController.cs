...
namespace WebApi.Controllers
{
    ...
    public class FarmController : ApiController
    {
        ...
        public  object GetFarms(string address)
        {
            ...
            //string str = string.Format("select * from tb_farm where address ->>'prov' like \'{0}\'and address ->> 'city' like \'{1}\' and address ->> 'coun' like \'{2}\'",
            //        s["prov"].ToString(), s["city"].ToString(), s["coun"].ToString());
        ...
        public object UpdateFarm(Farm farm)
        {
            ...
            {
                string str_update = "update tb_farm set \"name\" = @name,address =@address";
                var trans = PostgreSQL.BeginTransaction();
                try
                {
